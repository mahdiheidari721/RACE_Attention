...
TEXT_CONFIG = {
    # data / model
    "max_len": 16_000,          # 16K context (change if you want)
    "vocab_limit": 50_000,
    "embed_dim": 256,
    "num_heads": 4,
    "mlp_dim": 1024,
...
MIN_LEN_FOR_TRAIN = 16_000       # require raw token length >= this
...
# (Optional) Extra raw-length stats directly from examples:
...
max_len  = TEXT_CONFIG["max_len"]   # 16_000
...
print_effective_length_stats(
    ...
    thresholds=(MIN_LEN_FOR_TRAIN, max_len),
)
...
def train_model_simple(
    ...
    out_path = f"arxiv_{attn_type}_16K.txt"
...
def run_experiment(attn_types, cfg):
    for attn_type in attn_types:
        print(f"\n=== Training {attn_type.upper()} on Arxiv 16K ===")
