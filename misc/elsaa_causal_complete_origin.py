...
class CausalRACEAttention(nn.Module):
    """
    Causal RACE Attention following Algorithm 2 of the RACE paper.

    Maintains running cumulative state per table:
        A_cum^(l)[r] = sum_{j <= t} phi_K^(l)[j, r]
        B_cum^(l)[r, d] = sum_{j <= t} phi_K^(l)[j, r] * V[j, d]
    Then:
        num_t = (1/L) sum_l phi_Q^(l)[t] @ B_cum^(l, t)
        den_t = (1/L) sum_l phi_Q^(l)[t] @ A_cum^(l, t)
        O_t = num_t / den_t
        d_race(t) = den_t  (used by ELSAA fusion)

    Causal convention: update with K[t] before reading num/den (j <= t inclusive).

    Implementation: chunked cumsum.  Within each chunk we materialize
    [B, H, L, C, R, D] which stays small if C is moderate (e.g. 256).
    """
    def __init__(self, d, h, drop, num_bits=4, num_tables=4, beta_init=1.0,
        ...
        # Random Gaussian hyperplanes per table: [L, num_bits, dk]
        planes = torch.randn(num_tables, num_bits, self.dk, device=device)
        self.register_buffer('planes', planes, persistent=False)

        # Hypercube corners {-1,+1}^P : [R, num_bits]
        corners = torch.tensor(
            list(itertools.product([-1.0, 1.0], repeat=num_bits)),
            device=device, dtype=torch.float32,
        )
        self.register_buffer('corners', corners, persistent=False)  # [R, P]

        # Trainable temperature beta (softmax sharpness over corners)
        self.log_beta = nn.Parameter(torch.log(torch.tensor(beta_init, dtype=torch.float32)))

        # Cache for last-forward statistics (for ELSAA fusion / logging)
        self.last_den = None

    def _phi(self, x):
        """Compute phi features for x:[B,H,T,dk] -> [B,H,L,T,R] softmax probs."""
        B, H, T, dk = x.shape
        # Project: x @ W^(l)^T  where planes:[L,P,dk]
        # -> [B,H,T,L,P]
        proj = torch.einsum('bhtd,lpd->bhtlp', x, self.planes)
        # tanh and corner alignment
        tan = torch.tanh(proj)  # [B,H,T,L,P]
        beta = self.log_beta.exp().clamp(1e-2, 20.0)
        logits = beta * torch.einsum('bhtlp,rp->bhtlr', tan, self.corners)  # [B,H,T,L,R]
        probs = F.softmax(logits, dim=-1)  # [B,H,T,L,R]
        # Permute to [B,H,L,T,R] for cumsum along T
        return probs.permute(0, 1, 3, 2, 4).contiguous()

    def forward(self, x, mask=None, return_den=False):
        B, T, _ = x.shape
        H, dk, L, R = self.h, self.dk, self.num_tables, self.R
        ...
        # Chunked cumulative state
        state_A = torch.zeros(B, H, L, R, device=x.device, dtype=phiK.dtype)
        state_B = torch.zeros(B, H, L, R, dk, device=x.device, dtype=phiK.dtype)
        ...
        for cs in range(0, T, self.chunk_size):
            ce = min(cs + self.chunk_size, T)
            c = ce - cs
            pK_c = phiK[:, :, :, cs:ce, :]  # [B,H,L,c,R]
            pQ_c = phiQ[:, :, :, cs:ce, :]
            V_c = V[:, :, cs:ce, :]  # [B,H,c,dk]

            # Local within-chunk cumulative
            local_A = torch.cumsum(pK_c, dim=3)  # [B,H,L,c,R]
            # B_local[t] = sum_{s<=t} phi_K[s] outer V[s]
            #   = cumsum over t of phi_K_c[:,:,:,t,:] (R) * V_c[:,:,t,:] (dk)
            # Outer product per token then cumsum
            outer = pK_c.unsqueeze(-1) * V_c.unsqueeze(2).unsqueeze(-2)
            # outer: [B,H,L,c,R,dk]
            local_B = torch.cumsum(outer, dim=3)

            # Effective cumulative at each pos t in chunk = state + local
            A_at_t = state_A.unsqueeze(3) + local_A  # [B,H,L,c,R]
            B_at_t = state_B.unsqueeze(3) + local_B  # [B,H,L,c,R,dk]

            # Read with phi_Q: num[t] = phi_Q[t] @ B_at_t[t]
            num_t = torch.einsum('bhltr,bhltrd->bhltd', pQ_c, B_at_t)  # [B,H,L,c,dk]
            den_t = torch.einsum('bhltr,bhltr->bhlt', pQ_c, A_at_t)    # [B,H,L,c]

            # Average over L tables
            num_avg = num_t.mean(dim=2)  # [B,H,c,dk]
            den_avg = den_t.mean(dim=2)  # [B,H,c]

            out_c = num_avg / den_avg.unsqueeze(-1).clamp_min(1e-6)
            out_chunks.append(out_c)
            den_chunks.append(den_avg)

            # Update state for next chunk
            state_A = state_A + local_A[:, :, :, -1, :]
            state_B = state_B + local_B[:, :, :, -1, :, :]

        out = torch.cat(out_chunks, dim=2)  # [B,H,T,dk]
        den = torch.cat(den_chunks, dim=2)  # [B,H,T]
        ...
        # d_race per token: average over heads
        d_race_token = den.mean(dim=1, keepdim=False).unsqueeze(-1).clamp_min(1e-6)  # [B,T,1]
        self.last_den = d_race_token.detach()
...
SHARED_CFG = dict(
    task          = "mqar",          # "mqar" | "arxiv" | "lra"
    length        = 8192,            # sequence length
    epochs        = 1,
    lr            = 3e-4,
    weight_decay  = 0.01,
    embed_dim     = 256,
...
EXPERIMENTS = [
    ...
    dict(method="elsaa",        batch_size=2,  grad_accum_steps=8),
    ...
    #dict(method="causal_race",  batch_size=2,  grad_accum_steps=8),
