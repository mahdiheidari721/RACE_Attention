...
class CausalRACEAttention(nn.Module):
    ...
    def forward(self, x, mask=None, return_den=False):
        ...
        # ---------- INTER-CHUNK (no Python loop) ----------
        # chunk_A[c] = Σ_{s in c} φ_K(s)                  in R^R
        chunk_A = phiK_c.sum(dim=4)                                   # [B,H,L,n,R]
        # chunk_B[c] = φ_K_c.T @ V_c                      in R^{R×dk}
        chunk_B = torch.einsum('bhlncr,bhncd->bhlnrd',
                               phiK_c, V_c)                            # [B,H,L,n,R,dk]

        # Exclusive cumsum over chunks (pre-chunk state at chunk c)
        state_A = F.pad(torch.cumsum(chunk_A, dim=3)[:, :, :, :-1, :],
                        (0, 0, 1, 0))                                  # [B,H,L,n,R]
        state_B = F.pad(torch.cumsum(chunk_B, dim=3)[:, :, :, :-1, :, :],
                        (0, 0, 0, 0, 1, 0))                            # [B,H,L,n,R,dk]

        # Inter-chunk reads: each token sees its pre-chunk state
        inter_num = torch.einsum('bhlncr,bhlnrd->bhlncd',
                                 phiQ_c, state_B)                      # [B,H,L,n,C,dk]
        inter_den = torch.einsum('bhlncr,bhlnr->bhlnc',
                                 phiQ_c, state_A)                      # [B,H,L,n,C]

        # ---------- INTRA-CHUNK (causal-shaped, L pre-averaged) ----------
        # Averaging M over L *before* the V matmul saves an L factor of memory
        # and is mathematically equivalent for the V-dependent term.
        if self.chunk_group_size is None or self.chunk_group_size >= n_chunks:
            # All chunks at once
            M_avg = torch.einsum('bhlncr,bhlnsr->bhncs',
                                 phiQ_c, phiK_c) / L                   # [B,H,n,C,C]
            ...
            # Process chunks in groups to bound peak memory of M
        ...
        # ---------- COMBINE ----------
        # inter is per-L (mean now); intra is already L-averaged
        ...
        # Per-token density for ELSAA fusion
...
EXPERIMENTS = [
    ...
    #dict(method="linear",       batch_size=1,  grad_accum_steps=8),
]
