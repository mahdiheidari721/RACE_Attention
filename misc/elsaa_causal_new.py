...
Attention types:
    - elsaa            : Causal sortLSH + Causal RACE + m_sparse fusion
...
Patterns follow vit.py / arxiv_64k.py non-causal implementations exactly.
============================================================================
"""
...
def _run_exact_sdpa_causal(Q, K, V, causal=False):
    """SDPA returning output only."""
    if Q.device.type == "cuda":
        Q16, K16, V16 = [t.to(dtype=torch.float16) for t in (Q, K, V)]
        with sdpa_kernel(SDPBackend.FLASH_ATTENTION):
            out = F.scaled_dot_product_attention(
                Q16, K16, V16, dropout_p=0.0, is_causal=causal)
...
def add_self_attentions_lse(attn1, lse1, attn2, lse2):
    ...
    Same straightforward log-sum-exp combination as in the working non-causal
    HyperLSHExactWithLogDenomAttention (no NaN guards, no m_stable).
    """
    if lse1.dim() == 4:
        lse1 = lse1.squeeze(-1)
    if lse2.dim() == 4:
...
def exact_attention_flash(query, key, value, scale=None, causal=False):
    ...
    # Kernel expects (B, T, H, D) in fp16
    ...
    # flash_attn_func = FlashAttnFunc.apply handles fwd+bwd correctly
    # Returns out: (B, T, H, D), lse: (B, H, seqlen_rounded)
    out_t, lse_padded = _flash_attn_triton(q, k, v, None, causal, scale)

    # Permute output back to (B, H, T, D)
    #out = out_t.permute(0, 2, 1, 3).to(in_dtype).contiguous()
    out = out_t.reshape(B, H, Tq, D).to(in_dtype)   # reinterpret, no copy
    lse = lse_padded[:, :, :Tq]   # (B, H, T)
...
class CausalExactAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        out = _run_exact_sdpa_causal(Q, K, V, causal=True)
...
class CausalRACEAttention(nn.Module):
    """Causal RACE Attention.

    Two call modes:
      A. forward(x, mask=, return_den=)            — standalone (has own W_O)
      B. forward_core(Q, K, V, mask=, return_den=) — ELSAA branch, no W_O
    """
    ...
    def forward_core(self, Q, K, V, mask=None, return_den=False):
        """Compute RACE on pre-projected Q,K,V. Returns per-head output [B,H,T,Dk]."""
        B, H, T, dk = Q.shape
    ...
    def forward(self, x, mask=None, return_den=False):
        ...
        if return_den:
            head_out, d_token = self.forward_core(Q, K, V, mask=mask, return_den=True)
        else:
            head_out = self.forward_core(Q, K, V, mask=mask, return_den=False)
...
class CausalHyperSparseAttention(nn.Module):
    ...
    Two call modes:
      A. forward(x, mask=, return_lse=)             — standalone (has own W_O)
      B. forward_core(Q, K, V, mask=, return_lse=)  — ELSAA branch, no W_O
    """
    ...
    # def _exact_attention(self, q, k, v, causal=False):
    #     if causal:
    #         # Triton kernel has confirmed causal output bug.
    #         # Base case chunks are ~8K tokens — SDPA handles this fine.
    #         return exact_attention_sdpa(q, k, v, scale=self.scale, causal=True)
    #     else:
    #         # Non-causal path: called on block_size=512 chunks inside sortLSH.
    #         # Triton confirmed correct here. This is the hot path.
    #         if _HAS_TRITON_FLASH and q.device.type == "cuda":
    #             return exact_attention_flash(q, k, v, scale=self.scale, causal=False)
    ...
    def forward_core(self, Q, K, V, mask=None, return_lse=False):
        """Compute sparse attention on pre-projected Q,K,V. Returns per-head output."""
        if mask is not None:
            keep = mask[:, None, :, None].to(Q.dtype)
            Q = Q * keep; K = K * keep; V = V * keep
    ...
    def forward(self, x, mask=None, return_lse=False):
        ...
        if return_lse:
            head_out, log_d_token = self.forward_core(Q, K, V, mask=mask, return_lse=True)
        else:
            head_out = self.forward_core(Q, K, V, mask=mask, return_lse=False)
...
class CausalELSAAAttention(nn.Module):
    ...
    Mirrors HyperRaceMExactAttention / HyperRaceLambdaDependentAttention from
    arxiv_64k.py: same log-space m_exact formula, same straightforward log-space
    arithmetic.

        Q, K, V  =  W_Q(x), W_K(x), W_V(x)
        head_sparse, log_d_sparse = sparse.forward_core(Q, K, V)
        head_race,   d_race       = race.forward_core(Q, K, V)
        m_sparse = d_sparse / (d_sparse + lambda * d_race + eps)   # log-space
        head_fused = g_sparse * m_sparse * head_sparse + g_race * head_race
        out = W_O(dropout(head_fused))

    Each ELSAA layer creates its own (W_Q, W_K, W_V, W_O, log_lambda, gate_mlp),
    so per-layer lambda is automatic.
    """
    ...
    def __init__(self, cfg, device="cpu"):
        ...
        # Branches (their internal Q/K/V/O exist for standalone modes but are
        # unused when called via ELSAA.forward_core).
        self.sparse = CausalHyperSparseAttention(
            d=d, h=h, drop=drop,
            num_bits=cfg.get("hyper_num_bits", 5),
            block_size=cfg.get("hyper_block_size", 512),
        ...
        # Lambda parameter (per-layer, since each layer creates its own ELSAA).
        if self.lambda_dep:
    ...
    def _compute_lambda(self, x):
        """Returns lambda tensor.

        - If lambda_dep: shape [B,T,1]
        - Else: scalar
        """
        if self.lambda_dep:
            with torch.no_grad():
    ...
    def forward(self, x, mask=None):
        ...
        head_sparse, log_d_sparse = self.sparse.forward_core(
            Q, K, V, mask=mask, return_lse=True)
        head_race, d_race = self.race.forward_core(
            Q, K, V, mask=mask, return_den=True)

        # Lambda (scalar or [B,T,1])
        lam = self._compute_lambda(x)

        # m_sparse in log-space (same pattern as HyperRaceMExactAttention)
        ...
        # Broadcast [B,T,1] -> [B,1,T,1] for per-head fusion
        ...
        head_fused = g_sparse_h * m_sparse_h * head_sparse + g_race_h * head_race
        ...
        if isinstance(lam, torch.Tensor):
            ...
            self.last_lambda = lam.detach() if hasattr(lam, "detach") else None
    ...
    def __init__(self, cfg, attn_type, device="cpu"):
        ...
        if attn_type == "exact":
            ...
            self.att = CausalHyperSparseAttention(
                ...
                min_seq_len=cfg.get("hyper_min_seq_len", 2048),
                qkv_bias=cfg.get("qkv_bias", False), device=device)
...
class CausalTransformerClassifier(nn.Module):
    """Causal transformer with classification head; pools the LAST valid token."""
    ...
    def forward(self, x, mask):
        ...
        last_h = h.gather(1, gather_idx).squeeze(1)
        return self.head(last_h)


class CausalTransformerLM(nn.Module):
    """Causal LM head for MQAR-style next-token prediction."""
...
# ============================================================================
# SECTION 12: LRA RETRIEVAL PAIR DATA
# ============================================================================
...
def collect_attn_stats(model, attn_type):
    """Collect ELSAA gate / m_sparse / lambda stats for wandb logging."""
    stats = {}
...
EXPERIMENTS = [
    dict(method="elsaa",        batch_size=2,  grad_accum_steps=16),
    #dict(method="elsaa_lambda", batch_size=2,  grad_accum_steps=8),
    #dict(method="causal_sparse",batch_size=4,  grad_accum_steps=4),
    dict(method="causal_race",  batch_size=2,  grad_accum_steps=16),
    #dict(method="exact",        batch_size=8,  grad_accum_steps=1),
    #dict(method="linear",       batch_size=4,  grad_accum_steps=8),
]


def _diagnose_flash():
    if not _HAS_TRITON_FLASH:
        print("[diagnose] No Triton flash — skipping."); return
    if not torch.cuda.is_available():
        print("[diagnose] No CUDA — skipping."); return
    ...
    # Test several T values: multiple of 128, not multiple of 128, and 1024 (your block case)
    for T in [512, 1024, 1025, 2048, 8192]:
        print(f"\n--- T={T}, B={B}, H={H}, D={D} ---")
        ...
        for causal in (False, True):
            ...
            # ── Reference: SDPA ──────────────────────────────────────────
            o_s, lse_s = exact_attention_sdpa(q, k, v, scale=scale, causal=causal)
 
            # ── Flash wrapper ─────────────────────────────────────────────
            o_f, lse_f = exact_attention_flash(q, k, v, scale=scale, causal=causal)
 
            out_diff = (o_s - o_f).abs().max().item()
            lse_diff = (lse_s - lse_f).abs().max().item()
 
            print(f"    [wrapper]  max|Δout|={out_diff:.4g}   max|Δlse|={lse_diff:.4g}")
            print(f"    lse_sdpa  range: [{lse_s.min().item():.3f}, {lse_s.max().item():.3f}]")
            print(f"    lse_flash range: [{lse_f.min().item():.3f}, {lse_f.max().item():.3f}]")
            print(f"    lse diff  median={( lse_f - lse_s).median().item():.5f}  "
                  f"(log(scale)={math.log(scale):.3f}, log2={math.log(2):.3f})")
 
            # ── Raw Triton output BEFORE any permute ──────────────────────
            # This tells us: is the bug in the kernel, or in the permute logic?
            q_thd = q.permute(0, 2, 1, 3).to(torch.float16).contiguous()  # (B,T,H,D)
            k_thd = k.permute(0, 2, 1, 3).to(torch.float16).contiguous()
            v_thd = v.permute(0, 2, 1, 3).to(torch.float16).contiguous()
 
            with torch.no_grad():
                out_t_raw, lse_raw = _flash_attn_triton(q_thd, k_thd, v_thd, None, causal, scale)
            # out_t_raw: (B, T, H, D) — kernel's native output, no permute applied yet
            # o_s is (B, H, T, D); permute to (B, T, H, D) to compare in kernel space
            o_s_thd = o_s.permute(0, 2, 1, 3).to(torch.float16)
            raw_diff = (out_t_raw - o_s_thd).abs().max().item()
            print(f"    [raw kernel, before permute]  max|Δout|={raw_diff:.4g}")
            if raw_diff > 0.1:
                print(f"    *** Kernel itself is WRONG for causal={causal}, T={T} ***")
            else:
                print(f"    *** Kernel output is correct; any error is in the wrapper ***")
 
            # ── Gradient flow ─────────────────────────────────────────────
            # Detach and re-create with grad for a clean backward
            q2 = q.detach().requires_grad_(True)
            k2 = k.detach().requires_grad_(True)
            v2 = v.detach().requires_grad_(True)
            o_f2, _ = exact_attention_flash(q2, k2, v2, scale=scale, causal=causal)
            loss = o_f2.sum()
            loss.backward()
            q_grad_max = q2.grad.abs().max().item() if q2.grad is not None else None
            k_grad_max = k2.grad.abs().max().item() if k2.grad is not None else None
            v_grad_max = v2.grad.abs().max().item() if v2.grad is not None else None
            print(f"    grad q={q_grad_max}  k={k_grad_max}  v={v_grad_max}")
            if q_grad_max is None:
                print(f"    *** BROKEN AUTOGRAD: q.grad is None — empty+copy_ bug ***")
#_diagnose_flash()
def main():
