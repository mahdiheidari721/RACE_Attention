...
VISION_CONFIG = {
    ...
    "data_root": "./data",
    "batch_size": 2,                    # safer default for Oxford/Flowers 512x512; increase if memory allows
    "img_size": 512,
    "patch_size": 4,
    "num_channels": 3,
    #"num_classes": 37,
...
def set_dataset_cfg(cfg, dataset_name):
    ...
    if dataset_name == "fashionmnist":
        ...
        cfg["num_classes"] = 10
        cfg["num_patches"] = 784
        # FashionMNIST is small enough for bigger batches.
...
def gather_4d_scores(x: torch.Tensor, idx: torch.Tensor):
    """
    x:   [B,H,T,S]
    idx: [B,H,Q]
    out: [B,H,Q,S]
...
class AngularLSHGray(nn.Module):
    ...
    def hash(self, mat: torch.Tensor):
        ...
        # Make encoder broadcastable to arbitrary leading dims.
...
class RaceSoftBuckets(nn.Module):
    """RACE soft bucket probabilities using shared Q/K projections."""
...
def gather_block_summary(block_summary: torch.Tensor, block_ids: torch.Tensor):
    """
    block_summary: [B,H,NB,...]
    block_ids:     [B,H,Q]
    returns:       [B,H,Q,...]
    """
    trailing = block_summary.shape[3:]
    idx = block_ids
    for _ in trailing:
        idx = idx.unsqueeze(-1)
    idx = idx.expand(-1, -1, -1, *trailing)
    return block_summary.gather(2, idx)


def race_residual_readout(
    pQ: torch.Tensor,
    pK: torch.Tensor,
    V: torch.Tensor,
    k_sort_idx: torch.Tensor,
    k_orig2sort: torch.Tensor,
    q_block_id: torch.Tensor,
    global_first: int,
    window: int,
    ...
    """
    Efficient RACE branch with numerator-side elimination in summary space.

    It keeps the same denominator philosophy as the previous code:
      O_race(i) = pQ(i) @ ((B_full - B_used(i)) / A_full)
    and returns residual denominator mass d_race = pQ(i) @ A_res(i), used only for m_exact.
    """
    B, H, T, S = pQ.shape
    D = V.size(-1)
    device = V.device
    ...
    # LSH sorted block summaries for used-key elimination.
    pK_sorted = gather_4d_scores(pK, k_sort_idx)                   # [B,H,T,S]
    V_sorted = gather_4d_tokens(V, k_sort_idx)                     # [B,H,T,D]
    T_pad = math.ceil(T / block_size) * block_size
    pad_len = T_pad - T
    if pad_len > 0:
        pK_sorted = F.pad(pK_sorted, (0, 0, 0, pad_len))
        V_sorted = F.pad(V_sorted, (0, 0, 0, pad_len))
    num_blocks = T_pad // block_size
    pKb = pK_sorted.view(B, H, num_blocks, block_size, S)
    Vb = V_sorted.view(B, H, num_blocks, block_size, D)
    A_lsh_block = pKb.sum(dim=3)                                   # [B,H,NB,S]
    B_lsh_block = torch.einsum("bhnts,bhntd->bhnsd", pKb, Vb)       # [B,H,NB,S,D]

    out = torch.empty(B, H, T, D, device=device, dtype=V.dtype)
    d_race = torch.empty(B, H, T, device=device, dtype=V.dtype)

    for qs in range(0, T, q_chunk):
        qe = min(qs + q_chunk, T)
        qidx = torch.arange(qs, qe, device=device)
        q_len = qe - qs
        gw_idx, gw_mask = build_gw_indices(qidx, T, global_first, window)  # [q,U]
        U = gw_idx.size(1)

        # Summary of global/window union.
        pK_gw = pK[:, :, gw_idx, :]                # [B,H,q,U,S]
        V_gw = V[:, :, gw_idx, :]                  # [B,H,q,U,D]
        mask_gw = gw_mask.view(1, 1, q_len, U).to(V.dtype)
        pK_gw_m = pK_gw * mask_gw.unsqueeze(-1)
        A_gw = pK_gw_m.sum(dim=3)                  # [B,H,q,S]
        B_gw = torch.einsum("bhqus,bhqud->bhqsd", pK_gw_m, V_gw)

        # LSH block summary for each query.
        bid = q_block_id[:, :, qs:qe].clamp(0, num_blocks - 1)       # [B,H,q]
        A_lsh = gather_block_summary(A_lsh_block, bid)               # [B,H,q,S]
        B_lsh = gather_block_summary(B_lsh_block, bid)               # [B,H,q,S,D]

        # Overlap between GW keys and the LSH block, so used set is a union.
        kpos = k_orig2sort[:, :, gw_idx]                             # [B,H,q,U]
        kblock = torch.div(kpos, block_size, rounding_mode="floor")
        overlap = (kblock == bid.unsqueeze(-1)) & gw_mask.view(1, 1, q_len, U)
        overlap_f = overlap.to(V.dtype)
        pK_ov = pK_gw * overlap_f.unsqueeze(-1)
        A_ov = pK_ov.sum(dim=3)                                      # [B,H,q,S]
        B_ov = torch.einsum("bhqus,bhqud->bhqsd", pK_ov, V_gw)

        A_used = A_gw + A_lsh - A_ov                                  # [B,H,q,S]
        B_used = B_gw + B_lsh - B_ov                                  # [B,H,q,S,D]

        A_res = (A_full[:, :, None, :] - A_used).clamp_min(EPS)        # [B,H,q,S]
        B_res = B_full[:, :, None, :, :] - B_used                     # [B,H,q,S,D]

        # Keep full denominator for branch output, matching old numerator-only elimination.
        E_res_full_den = B_res / A_full[:, :, None, :, None]
        qprob = pQ[:, :, qs:qe, :]                                    # [B,H,q,S]
        out[:, :, qs:qe, :] = torch.einsum("bhqs,bhqsd->bhqd", qprob, E_res_full_den)
        d_race[:, :, qs:qe] = torch.einsum("bhqs,bhqs->bhq", qprob, A_res)

    return out, d_race


class HyperPlusPlusRaceAttentionVision(nn.Module):
    """
    hyper_plus_plus_race
    ...
    RACE branch:
      - computes RACE summaries once
      - removes exact-used keys in summary space before readout
      - keeps full RACE denominator for the output, like previous numerator-only exclusion
    ...
    def __init__(self, cfg, device="cpu"):
        ...
        self.race_q_chunk = cfg.get("race_q_chunk_size", 64)
    ...
    def forward(self, x):
        ...
        # RACE branch with summary-space used-key elimination.
        pQ, pK = self.race_buckets(Q, K)  # [B,H,T,S]
        out_race, d_race = race_residual_readout(
            pQ, pK, V,
            k_sort_idx=meta["k_sort_idx"],
            k_orig2sort=meta["k_orig2sort"],
            q_block_id=meta["q_block_id"],
            global_first=self.global_first,
            window=self.window,
            block_size=self.block_size,
            q_chunk=self.race_q_chunk,
        )
...
class HyperPlusPlusRaceBlock(nn.Module):
    def __init__(self, cfg, device="cpu"):
        super().__init__()
        d = cfg["embed_dim"]
        self.att = HyperPlusPlusRaceAttentionVision(cfg, device=device)
        self.norm1 = nn.LayerNorm(d)
        self.norm2 = nn.LayerNorm(d)
        self.ff = nn.Sequential(nn.Linear(d, 4 * d), nn.GELU(), nn.Linear(4 * d, d))
        self.drop = nn.Dropout(cfg["drop_rate"])
...
class VisionTransformer(nn.Module):
    def __init__(self, cfg, attn_type, device="cpu"):
        ...
        if attn_type == "hyper_plus_plus_race":
            Block = lambda c: HyperPlusPlusRaceBlock(c, device=device)
    ...
    with open(out_path, "a", encoding="utf-8") as f:
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for images, labels in train_iter:
                ...
                train_iter.set_postfix({"loss": running_loss / max(1, running_total / labels.numel()), "acc": running_correct / max(1, running_total)})
            ...
            with torch.no_grad():
                ...
                for images, labels in val_iter:
                    ...
                    val_iter.set_postfix({"loss": val_loss / max(1, len(val_iter)), "acc": val_correct / max(1, val_total)})
            ...
            extra_logs = collect_hpr_stats(model) if attn_type == "hyper_plus_plus_race" else {}
            ...
            _log(f, f"Ep{epoch:3d} | train_loss {tr_l:.4f}, acc {tr_a:.4f} ({train_time:.1f}s) | val_loss {va_l:.4f}, acc {va_a:.4f} ({val_time:.1f}s) | lr {lr:.3e} | updates {global_update}/{total_updates}")
...
def start_experiment():
    ...
    experiments = [
        ("hyper_plus_plus_race", cfg["grad_accum_steps"]),
        # ("exact_flash", 1),
    ]
