...
def flash_attention(Q, K, V, eps=1e-6):
    """
    FlashAttention-3 wrapper (non-causal) using Dao et al.'s flash-attn library.

    Q,K,V: [B,H,T,D] -> [B,H,T,D]
    """
    if flash_attn_func is None:
        raise RuntimeError(
            "flash_attn is not installed or not importable. "
            "Install FlashAttention-2 in this environment first using pip install flash-attn."
        )
    ...
    out = flash_attn_func(
        q, k, v,
        dropout_p=0.0,
        softmax_scale=None,   # uses default 1/sqrt(D)
        causal=False          # non-causal, like your other kernels
    )  # [B,T,H,D]

    return out.transpose(1, 2).contiguous()  # [B,H,T,D]
...
def main():
    ...
    p.add_argument('--methods', nargs='+',
                   default=['softmax', 'flash'],
                   choices=['softmax', 'flash'],)
    p.add_argument('--angular_exp', type=float, default=6.0)
    p.add_argument('--linformer_k', type=int, default=128)
    p.add_argument('--race_L', type=int, default=3)
    p.add_argument('--race_Kbits', type=int, default=3)
